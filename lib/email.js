  ...
  if (order.paymentMethod === 'SINPE') {
    paymentInstructions = `
      <div style="background-color: #f8f9fa; padding: 15px; border-radius: 8px; margin: 20px 0;">
        ...
        <p><strong>Teléfono:</strong> 7097-3376</p>
    ...
    paymentInstructions = `
      <div style="background-color: #f8f9fa; padding: 15px; border-radius: 8px; margin: 20px 0;">
        ...
        <p><strong>Cuenta:</strong> CR66010200009691875099 a nombre de Eddyn Gerardo Soto Arguedas</p>
        <p>Asegúrate de enviar el comprobante e incluir el ID de tu orden en el detalle del pago.</p>
      </div>
    `;
  } else {
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
      ...
      <div style="padding: 20px; border: 1px solid #ddd; border-top: none; border-radius: 0 0 8px 8px;">
        ...
        <div style="text-align: center; margin: 30px 0;">
          <a href="https://wa.me/50670973376?text=Hola%20tengo%20una%20consulta%20sobre%20mi%20orden%20%23${order.id.split('-')[0].toUpperCase()}" style="display: inline-block; background-color: #25D366; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
...
export async function sendAdminNewOrderEmail(order) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
      ...
      <div style="padding: 20px; border: 1px solid #ddd; border-top: none; border-radius: 0 0 8px 8px;">
        <p>Se ha registrado una nueva orden en el sistema de Motopillos.</p>
        <div style="background-color: #f8f9fa; padding: 15px; border-radius: 8px; margin: 20px 0;">
          <p style="margin: 0;"><strong>Orden ID:</strong> #${order.id.split('-')[0].toUpperCase()}</p>
          <p style="margin: 5px 0 0 0;"><strong>Cliente:</strong> ${order.customerName}</p>
          <p style="margin: 5px 0 0 0;"><strong>Email:</strong> ${order.customerEmail}</p>
          <p style="margin: 5px 0 0 0;"><strong>Teléfono:</strong> ${order.customerPhone}</p>
          <p style="margin: 5px 0 0 0;"><strong>Método de Pago:</strong> ${order.paymentMethod}</p>
          <p style="margin: 5px 0 0 0;"><strong>Total (USD):</strong> $${order.totalAmount.toFixed(2)}</p>
        </div>
        <div style="text-align: center; margin: 30px 0;">
          <a href="https://motopillos.com/admin/orders" style="display: inline-block; background-color: #ef4444; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
  ...
  try {
    await transporter.sendMail({
      from: '"Motopillos Admin" <' + process.env.SMTP_USER + '>',
      to: process.env.ADMIN_EMAIL || 'motopillos@gmail.com',
      subject: `NUEVA ORDEN: #${order.id.split('-')[0].toUpperCase()} - ${order.customerName}`,
      html: html,
    });
...
export async function sendPasswordResetEmail(user, token) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
      ...
      <div style="padding: 20px; border: 1px solid #ddd; border-top: none; border-radius: 0 0 8px 8px;">
        ...
        <p>Hola ${user.name},</p>
        <p>Hemos recibido una solicitud para restablecer la contraseña de tu cuenta en Motopillos asociada a este correo electrónico.</p>
        
        <div style="text-align: center; margin: 30px 0;">
          <a href="${resetLink}" style="display: inline-block; background-color: #3b82f6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
            Restablecer Mi Contraseña
          </a>
        </div>

        <p style="color: #666; font-size: 0.9em;">Si no solicitaste este cambio, puedes ignorar este correo de forma segura. El enlace expirará en 1 hora.</p>
      </div>
    </div>
  `;
  ...
  try {
    await transporter.sendMail({
      from: '"Motopillos" <' + process.env.SMTP_USER + '>',
      to: user.email,
      subject: 'Recuperación de Contraseña - Motopillos',
      html: html,
      attachments: [{
        filename: 'logo.png',
        path: process.cwd() + '/public/logo.png',
